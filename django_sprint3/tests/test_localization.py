...
def test_models_translated(n_model, n_verbose, n_verbose_plural):
    ...
    for model in models:
        if model._meta.object_name != n_model:
            continue
        found_model.append(model)
        print(model.__name__, hasattr(model, "Meta"), model._meta.verbose_name)
    # found_model = [
    ...
    assert found_model, f"Убедитесь, что в модели `{n_model}` описан подкласс `Meta`."
    ...
    assert found_model._meta.verbose_name_plural == n_verbose_plural, (
